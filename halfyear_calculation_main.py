...
def Get_file_list(file_path ,qybg_paths ,fzjg_paths ,qygx_paths ,qyb_paths):
    for path in file_path:
        for root , dir , files in os.walk(path):
            if len(dir) == 0:
                if "变更情况表" in root and '.zip' not in root:
                    qybg_paths.append(os.path.join(root ,files[0]))
                if "分支机构表" in root and '.zip' not in root:
                    fzjg_paths.append(os.path.join(root ,files[0]))
                if "企业关系人表" in root and '.zip' not in root:
                    qygx_paths.append(os.path.join(root, files[0]))
                if "迁移信息表" in root and '.zip' not in root:
                    qyb_paths.append(os.path.join(root, files[0]))

def Get_ztdj(file_path):
    logger.info(os.listdir(file_path))
    for listname in os.listdir(file_path):
        if "主体登记" in listname and ".zip" not in listname:
            dir_path = os.path.join(file_path,listname)
            basename = os.listdir(dir_path)[0]
            ztdj_path = os.path.join(dir_path ,basename)
            return ztdj_path

    logger.info("缺少主体登记文件")
    exit(0)

def Create_zippedfile(output_path):

    zip_file_path = output_path + ".ZIP"
    logger.info("生成结果ZIP文件 ,文件路径为 ：" + zip_file_path)
    logger.info("SUCCESS_ZIP_FILE_PATH=" + zip_file_path)
    with zipfile.ZipFile(zip_file_path, 'w', zipfile.ZIP_DEFLATED) as zip_ref:
        for root, _, files in os.walk(output_path):
            for file in files:
                path = os.path.join(root, file)
                arc_name = os.path.relpath(path, output_path)
...
if __name__ == '__main__':
    ...
    month1_data_path = sys.argv[1]
    month2_data_path = sys.argv[2]
    month3_data_path = sys.argv[3]
    month4_data_path = sys.argv[4]
    month5_data_path = sys.argv[5]
    month6_data_path = sys.argv[6]
    # output_path = sys.argv[7] #输出文件夹

    dir = os.path.dirname(month1_data_path)
    logger.info(dir)
    output_path = os.path.join(dir ,"output")
    logger.info(output_path)
    if not os.path.exists(output_path):
        os.makedirs(output_path)
        print("create output_file floder")
    else:
        print("the floder already exists")


    data_path = [month1_data_path ,month2_data_path ,month3_data_path ,month4_data_path ,month5_data_path ,month6_data_path]
    ztdj_path = Get_ztdj(month6_data_path) #末月主体等级表
    logger.info(ztdj_path)
    start_month = os.path.basename(month1_data_path)#开始年月
