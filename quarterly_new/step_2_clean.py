...
def process(quarter_source_data_files, result_data_dir, cache_dir, quarter):
    ...
    file = pd.read_csv(quarter_third_month_data["主体登记"], encoding='gb18030')
    ...
    new_company.info(show_counts=True) #提取出本季度末月的数据
    ...
    print(revoke.info(show_counts=True))
    ...
    # In[14]:
    #此处以后的 new_file 表 为 new_file表 与 new_revoke表的 合并
    ...
    # In[29]:
    year = int(quarter[0].split('-')[0])

    new_file['time'] = year - new_file['clrq'].str[:4].astype(np.int64)
    ...
    # In[51]:
    qybg_file_path = os.path.join(cache_dir, "企业变更.csv")
    qybg_file = pd.read_csv(qybg_file_path)
    rename_frame_columns_name(qybg_file, 0, '#NBXH(C|36)')
    rename_frame_columns_name(qybg_file, 1, 'BGSX(C|64)')
    rename_frame_columns_name(qybg_file, 2, 'CQRQ(P)')
    ...
    bgsx_M = list(np.array([120, 170, 700, 134, 118, 155, 117, 127, 136, 154, 914, 131, 923],dtype=np.float64))
    ...
    qybg_file_M.info(show_counts=True)
    ...
    print("*"*100)
    print(qybg_file[qybg_file['BGSX(C|64)'] == 165])
    print("*" * 100)
    ...
    fzjg_file_path = os.path.join(cache_dir, "分支机构.csv")
    fzjg_file = pd.read_csv(fzjg_file_path)
    ...
    # In[84]:
    invest_file_path = os.path.join(cache_dir, "企业关系人.csv")
    invest = pd.read_csv(invest_file_path)
    ...
    # In[94]:
    qycs_file_path = os.path.join(cache_dir, "迁移.csv")
    qycs_file = pd.read_csv(qycs_file_path)
    ...
    path = os.path.join(cache_dir, 'DataCleaning_quarterTwo_1.csv')
