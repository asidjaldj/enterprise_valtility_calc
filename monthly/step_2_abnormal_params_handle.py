...
def process(cache_dir):
    # # 读取

    # In[2]:

    path_dc = os.path.join(cache_dir, 'DataCleaning_revised_1.csv')
    ...
    for i in tqdm(alphabet):
        ...
        temp_group = eval(temp_alpha)
        temp_group.set_index('nbxh', inplace=True)
        temp_group = temp_group[feature_list]  # 提取出需要更改的列
        # 更改列名
        temp_group.rename(
            columns={'X5': 'X5_fill', 'X6': 'X6_fill', 'X7': 'X7_fill', 'X10': 'X10_fill',
                     'X11': 'X11_fill', 'X12': 'X12_fill', 'X13': 'X13_fill', 'X14': 'X14_fill', 'X18': 'X18_fill'},
            inplace=True)
        dc = dc.join(temp_group)
    ...
    path_para = os.path.join(cache_dir, 'para_1.csv')
    ...
    path_para = os.path.join(cache_dir, 'para_2.csv')
    ...
    path_dataclean_2 = os.path.join(cache_dir, 'DataCleaning_revised_2.csv')
    dc.to_csv(path_dataclean_2)


if __name__ == '__main__':
    process(r"D:\codes\bigdata\2023第一季\2023-02\cache1704297940")
