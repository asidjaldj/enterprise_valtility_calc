...
def process(cache_dir, data_dir):
    # In[3]:

    # i = datetime.datetime.now()
    # time_str = str(i.year) + '_' + str(i.month) + '_' + str(i.day) + '_hour' + str(i.hour)
    # # 定义要创建的文件夹
    # mkpath = '.\\' + time_str
    # # 调用函数
    # mkdir(mkpath)
    ...
    # path_dc = './DataCleaning_revised_2.csv'
    path_dc = os.path.join(cache_dir, 'DataCleaning_revised_2.csv')
    ...
    # path_para_1 = './para_1.csv'
    path_para_1 = os.path.join(cache_dir, 'para_1.csv')
    industry = pd.read_csv(path_para_1, index_col=[0])

    # In[6]:

    # path_para_2 = './para_2.csv'
    path_para_2 = os.path.join(cache_dir, 'para_2.csv')
    ...
    all_file = os.path.join(data_dir, '整体活跃度.txt')
    ...
    df_industry.to_csv(os.path.join(data_dir, '行业活跃度.csv'))
    ...
    df_scale.to_csv(os.path.join(data_dir, '规模活跃度.csv'))
    ...
    df_city.to_csv(os.path.join(data_dir, '地区活跃度.csv'))
    ...
    df_three.to_csv(os.path.join(data_dir, '三次产业活跃度.csv'))
    ...
    df_economic.to_csv(os.path.join(data_dir, '经济户口活跃度.csv'))
    ...
    df_time.to_csv(os.path.join(data_dir, '企业存续时间活跃度.csv'))
    ...
    df_first.to_csv(os.path.join(data_dir, '分行业：一产行业数量及占比.csv'))
    df_second.to_csv(os.path.join(data_dir, '分行业：二产行业数量及占比.csv'))
    df_third.to_csv(os.path.join(data_dir, '分行业：三产行业数量及占比.csv'))
